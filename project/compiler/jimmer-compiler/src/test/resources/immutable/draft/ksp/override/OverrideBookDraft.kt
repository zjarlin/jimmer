@file:Suppress("warnings")

package demo

import com.fasterxml.jackson.`annotation`.JsonIgnore
import com.fasterxml.jackson.`annotation`.JsonPropertyOrder
import java.io.Serializable
import java.lang.IllegalStateException
import kotlin.Any
import kotlin.Boolean
import kotlin.Cloneable
import kotlin.Int
import kotlin.Long
import kotlin.String
import kotlin.Suppress
import kotlin.Unit
import kotlin.collections.MutableList
import org.babyfish.jimmer.CircularReferenceException
import org.babyfish.jimmer.DraftConsumer
import org.babyfish.jimmer.ImmutableObjects
import org.babyfish.jimmer.UnloadedException
import org.babyfish.jimmer.`internal`.GeneratedBy
import org.babyfish.jimmer.jackson.ImmutableModuleRequiredException
import org.babyfish.jimmer.kt.DslScope
import org.babyfish.jimmer.kt.ImmutableCreator
import org.babyfish.jimmer.meta.ImmutableType
import org.babyfish.jimmer.meta.PropId
import org.babyfish.jimmer.runtime.DraftContext
import org.babyfish.jimmer.runtime.DraftSpi
import org.babyfish.jimmer.runtime.ImmutableSpi
import org.babyfish.jimmer.runtime.Internal
import org.babyfish.jimmer.runtime.Visibility

@DslScope
@GeneratedBy(type = OverrideBook::class)
public interface OverrideBookDraft : OverrideBook, BaseOnlyOneSwitchDraft {
    override var id: Long

    @GeneratedBy(type = OverrideBook::class)
    public object `$` {
        public const val SLOT_ID: Int = 1

        public const val SLOT_STATUS: Int = 0

        public val type: ImmutableType = ImmutableType
            .newBuilder(
                "0.11.6",
                OverrideBook::class,
                listOf(
                    BaseOnlyOneSwitchDraft.`$`.type
                ),
            ) { ctx, base ->
                DraftImpl(ctx, base as OverrideBook?)
            }
            .redefine("status", SLOT_STATUS)
            .id(SLOT_ID, "id", Long::class.java)
            .build()

        public fun produce(base: OverrideBook? = null, resolveImmediately: Boolean = false): OverrideBook {
            val consumer = DraftConsumer<OverrideBookDraft> {}
            return Internal.produce(type, base, resolveImmediately, consumer) as OverrideBook
        }

        public fun produce(
            base: OverrideBook? = null,
            resolveImmediately: Boolean = false,
            block: OverrideBookDraft.() -> Unit,
        ): OverrideBook {
            val consumer = DraftConsumer<OverrideBookDraft> { block(it) }
            return Internal.produce(type, base, resolveImmediately, consumer) as OverrideBook
        }

        @GeneratedBy(type = OverrideBook::class)
        @JsonPropertyOrder("dummyPropForJacksonError__", "status", "id")
        private abstract interface Implementor : OverrideBook, ImmutableSpi {
            public val dummyPropForJacksonError__: Int
                get() = throw ImmutableModuleRequiredException()

            override fun __get(prop: PropId): Any? = when (prop.asIndex()) {
                -1 ->
                	__get(prop.asName())
                SLOT_STATUS ->
                	status
                SLOT_ID ->
                	id
                else -> throw IllegalArgumentException(
                    "Illegal property name" + 
                    " for \"demo.OverrideBook\": " + 
                    prop
                )

            }

            override fun __get(prop: String): Any? = when (prop) {
                "status" ->
                	status
                "id" ->
                	id
                else -> throw IllegalArgumentException(
                    "Illegal property name" + 
                    " for \"demo.OverrideBook\": " + 
                    prop
                )

            }

            override fun __type(): ImmutableType = `$`.type
        }

        @GeneratedBy(type = OverrideBook::class)
        private class Impl : Implementor, Cloneable, Serializable {
            @get:JsonIgnore
            internal var __visibility: Visibility? = null

            @get:JsonIgnore
            internal var __idValue: Long = 0

            @get:JsonIgnore
            internal var __idLoaded: Boolean = false

            @get:JsonIgnore
            internal var __statusValue: Int = 0

            @get:JsonIgnore
            internal var __statusLoaded: Boolean = false

            override val id: Long
                get() {
                    if (!__idLoaded) {
                        throw UnloadedException(OverrideBook::class.java, "id")
                    }
                    return __idValue
                }

            override val status: Int
                get() {
                    if (!__statusLoaded) {
                        throw UnloadedException(OverrideBook::class.java, "status")
                    }
                    return __statusValue
                }

            public override fun clone(): Impl {
                val copy = super.clone() as Impl
                val originalVisibility = this.__visibility
                if (originalVisibility != null) {
                    val newVisibility = Visibility.of(2)
                    for (propId in 0 until 2) {
                        newVisibility.show(propId, originalVisibility.visible(propId))
                    }
                    copy.__visibility = newVisibility
                } else {
                    copy.__visibility = null
                }
                return copy
            }

            override fun __isLoaded(prop: PropId): Boolean = when (prop.asIndex()) {
                -1 ->
                	__isLoaded(prop.asName())
                SLOT_STATUS ->
                	__statusLoaded
                SLOT_ID ->
                	__idLoaded
                else -> throw IllegalArgumentException(
                    "Illegal property name" + 
                    " for \"demo.OverrideBook\": " + 
                    prop
                )

            }

            override fun __isLoaded(prop: String): Boolean = when (prop) {
                "status" ->
                	__statusLoaded
                "id" ->
                	__idLoaded
                else -> throw IllegalArgumentException(
                    "Illegal property name" + 
                    " for \"demo.OverrideBook\": " + 
                    prop
                )

            }

            override fun __isVisible(prop: PropId): Boolean {
                val __visibility = this.__visibility ?: return true
                return when (prop.asIndex()) {
                    -1 ->
                    	__isVisible(prop.asName())
                    SLOT_STATUS ->
                    	__visibility.visible(SLOT_STATUS)
                    SLOT_ID ->
                    	__visibility.visible(SLOT_ID)
                    else -> true
                }
            }

            override fun __isVisible(prop: String): Boolean {
                val __visibility = this.__visibility ?: return true
                return when (prop) {
                    "status" ->
                    	__visibility.visible(SLOT_STATUS)
                    "id" ->
                    	__visibility.visible(SLOT_ID)
                    else -> true
                }
            }

            public fun __shallowHashCode(): Int {
                var hash = __visibility?.hashCode() ?: 0
                if (__idLoaded) {
                    hash = 31 * hash + __idValue.hashCode()
                }
                if (__statusLoaded) {
                    hash = 31 * hash + __statusValue.hashCode()
                }
                return hash
            }

            override fun hashCode(): Int {
                var hash = __visibility?.hashCode() ?: 0
                if (__idLoaded) {
                    hash = 31 * hash + __idValue.hashCode()
                    return hash
                }
                if (__statusLoaded) {
                    hash = 31 * hash + __statusValue.hashCode()
                }
                return hash
            }

            override fun __hashCode(shallow: Boolean): Int = if (shallow) __shallowHashCode() else hashCode()

            public fun __shallowEquals(other: Any?): Boolean {
                val __other = other as? Implementor
                if (__other === null) {
                    return false
                }
                if (__isVisible(PropId.byIndex(SLOT_ID)) != __other.__isVisible(PropId.byIndex(SLOT_ID))) {
                    return false
                }
                val __idLoaded = 
                    this.__idLoaded
                if (__idLoaded != (__other.__isLoaded(PropId.byIndex(SLOT_ID)))) {
                    return false
                }
                if (__idLoaded && this.__idValue != __other.id) {
                    return false
                }
                if (__isVisible(PropId.byIndex(SLOT_STATUS)) != __other.__isVisible(PropId.byIndex(SLOT_STATUS))) {
                    return false
                }
                val __statusLoaded = 
                    this.__statusLoaded
                if (__statusLoaded != (__other.__isLoaded(PropId.byIndex(SLOT_STATUS)))) {
                    return false
                }
                if (__statusLoaded && this.__statusValue != __other.status) {
                    return false
                }
                return true
            }

            override fun equals(other: Any?): Boolean {
                val __other = other as? Implementor
                if (__other === null) {
                    return false
                }
                if (__isVisible(PropId.byIndex(SLOT_ID)) != __other.__isVisible(PropId.byIndex(SLOT_ID))) {
                    return false
                }
                val __idLoaded = 
                    this.__idLoaded
                if (__idLoaded != (__other.__isLoaded(PropId.byIndex(SLOT_ID)))) {
                    return false
                }
                if (__idLoaded) {
                    return this.__idValue == __other.id
                }
                if (__isVisible(PropId.byIndex(SLOT_STATUS)) != __other.__isVisible(PropId.byIndex(SLOT_STATUS))) {
                    return false
                }
                val __statusLoaded = 
                    this.__statusLoaded
                if (__statusLoaded != (__other.__isLoaded(PropId.byIndex(SLOT_STATUS)))) {
                    return false
                }
                if (__statusLoaded && this.__statusValue != __other.status) {
                    return false
                }
                return true
            }

            override fun __equals(obj: Any?, shallow: Boolean): Boolean = if (shallow) __shallowEquals(obj) else equals(obj)

            override fun toString(): String = ImmutableObjects.toString(this)
        }

        @GeneratedBy(type = OverrideBook::class)
        internal class DraftImpl(
            ctx: DraftContext?,
            base: OverrideBook?,
        ) : Implementor,
            OverrideBookDraft,
            DraftSpi {
            private val __ctx: DraftContext? = ctx

            private val __base: Impl? = base as Impl?

            private var __modified: Impl? = if (base === null) Impl() else null

            private var __resolving: Boolean = false

            private var __resolved: OverrideBook? = null

            override var id: Long
                get() = (__modified ?: __base!!).id
                set(id) {
                    if (__resolved != null) {
                        throw IllegalStateException("The current draft has been resolved so it cannot be modified")
                    }
                    val __tmpModified = (__modified ?: __base!!.clone())
                            .also { __modified = it }
                    __tmpModified.__idValue = id
                    __tmpModified.__idLoaded = true
                }

            override var status: Int
                get() = (__modified ?: __base!!).status
                set(status) {
                    if (__resolved != null) {
                        throw IllegalStateException("The current draft has been resolved so it cannot be modified")
                    }
                    val __tmpModified = (__modified ?: __base!!.clone())
                            .also { __modified = it }
                    __tmpModified.__statusValue = status
                    __tmpModified.__statusLoaded = true
                }

            override fun __isLoaded(prop: PropId): Boolean = (__modified ?: __base!!).__isLoaded(prop)

            override fun __isLoaded(prop: String): Boolean = (__modified ?: __base!!).__isLoaded(prop)

            override fun __isVisible(prop: PropId): Boolean = (__modified ?: __base!!).__isVisible(prop)

            override fun __isVisible(prop: String): Boolean = (__modified ?: __base!!).__isVisible(prop)

            override fun hashCode(): Int = (__modified ?: __base!!).hashCode()

            override fun __hashCode(shallow: Boolean): Int = (__modified ?: __base!!).__hashCode(shallow)

            override fun equals(other: Any?): Boolean = (__modified ?: __base!!).equals(other)

            override fun __equals(other: Any?, shallow: Boolean): Boolean = (__modified ?: __base!!).__equals(other, shallow)

            override fun toString(): String = ImmutableObjects.toString(this)

            override fun __unload(prop: PropId) {
                if (__resolved != null) {
                    throw IllegalStateException("The current draft has been resolved so it cannot be modified")
                }
                when (prop.asIndex()) {
                    -1 ->
                    	__unload(prop.asName())
                    SLOT_STATUS ->
                    	 {
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__statusValue = 0
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__statusLoaded = false
                        }
                    SLOT_ID ->
                    	 {
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__idValue = 0
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__idLoaded = false
                        }
                    else -> throw IllegalArgumentException(
                        "Illegal property name" + 
                        " for \"demo.OverrideBook\": " + 
                        prop
                    )

                }
            }

            override fun __unload(prop: String) {
                if (__resolved != null) {
                    throw IllegalStateException("The current draft has been resolved so it cannot be modified")
                }
                when (prop) {
                    "status" ->
                    	 {
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__statusValue = 0
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__statusLoaded = false
                        }
                    "id" ->
                    	 {
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__idValue = 0
                            (__modified ?: __base!!.clone())
                                    .also { __modified = it }
                                    .__idLoaded = false
                        }
                    else -> throw IllegalArgumentException(
                        "Illegal property name" + 
                        " for \"demo.OverrideBook\": " + 
                        prop
                    )

                }
            }

            override fun __set(prop: PropId, `value`: Any?) {
                when (prop.asIndex()) {
                    -1 ->
                    	__set(prop.asName(), value)
                    SLOT_STATUS ->
                    	this.status = value as Int?
                    	?: throw IllegalArgumentException("'status cannot be null")
                    SLOT_ID ->
                    	this.id = value as Long?
                    	?: throw IllegalArgumentException("'id cannot be null")
                    else -> throw IllegalArgumentException(
                        "Illegal property name" + 
                        " for \"demo.OverrideBook\": " + 
                        prop
                    )

                }
            }

            override fun __set(prop: String, `value`: Any?) {
                when (prop) {
                    "status" ->
                    	this.status = value as Int?
                    	?: throw IllegalArgumentException("'status cannot be null")
                    "id" ->
                    	this.id = value as Long?
                    	?: throw IllegalArgumentException("'id cannot be null")
                    else -> throw IllegalArgumentException(
                        "Illegal property name" + 
                        " for \"demo.OverrideBook\": " + 
                        prop
                    )

                }
            }

            override fun __show(prop: PropId, visible: Boolean) {
                if (__resolved != null) {
                    throw IllegalStateException("The current draft has been resolved so it cannot be modified")
                }
                val __visibility = (__modified ?: __base!!).__visibility
                    ?: if (visible) {
                        null
                    } else {
                        Visibility.of(2).also{
                            (__modified ?: __base!!.clone())
                            .also { __modified = it }.__visibility = it}
                    }
                    ?: return
                when (prop.asIndex()) {
                    -1 ->
                    	__show(prop.asName(), visible)
                    SLOT_STATUS ->
                    	__visibility.show(SLOT_STATUS, visible)
                    SLOT_ID ->
                    	__visibility.show(SLOT_ID, visible)
                    else -> throw IllegalArgumentException(
                        "Illegal property id: \"" + 
                        prop + 
                        "\",it does not exists"
                    )
                }
            }

            override fun __show(prop: String, visible: Boolean) {
                if (__resolved != null) {
                    throw IllegalStateException("The current draft has been resolved so it cannot be modified")
                }
                val __visibility = (__modified ?: __base!!).__visibility
                    ?: if (visible) {
                        null
                    } else {
                        Visibility.of(2).also{
                            (__modified ?: __base!!.clone())
                            .also { __modified = it }.__visibility = it}
                    }
                    ?: return
                when (prop) {
                    "status" ->
                    	__visibility.show(SLOT_STATUS, visible)
                    "id" ->
                    	__visibility.show(SLOT_ID, visible)
                    else -> throw IllegalArgumentException(
                        "Illegal property name: \"" + 
                        prop + 
                        "\",it does not exists"
                    )
                }
            }

            override fun __draftContext(): DraftContext = __ctx()

            override fun __resolve(): Any {
                val __resolved = this.__resolved
                if (__resolved != null) {
                    return __resolved
                }
                if (__resolving) {
                    throw CircularReferenceException()
                }
                __resolving = true
                val __ctx = __ctx()
                try {
                    val base = __base
                    var __tmpModified = __modified
                    if (base !== null && __tmpModified === null) {
                        this.__resolved = base
                        return base
                    }
                    this.__resolved = __tmpModified
                    return __tmpModified!!
                } finally {
                    __resolving = false
                }
            }

            override fun __isResolved(): Boolean = __resolved != null

            private fun __ctx(): DraftContext = __ctx ?: error("The current draft object is simple draft which does not support converting nested object to nested draft")

            internal fun __unwrap(): Any = __modified ?: error("Internal bug, draft for builder must have `__modified`")
        }
    }

    @GeneratedBy(type = OverrideBook::class)
    public class Builder {
        private val __draft: `$`.DraftImpl

        public constructor(base: OverrideBook?) {
            __draft = `$`.DraftImpl(null, base)
        }

        public constructor() : this(null)

        public fun id(id: Long?): Builder {
            if (id !== null) {
                __draft.id = id
                __draft.__show(PropId.byIndex(`$`.SLOT_ID), true)
            }
            return this
        }

        public fun status(status: Int?): Builder {
            if (status !== null) {
                __draft.status = status
                __draft.__show(PropId.byIndex(`$`.SLOT_STATUS), true)
            }
            return this
        }

        public fun build(): OverrideBook = __draft.__unwrap() as OverrideBook
    }
}

@GeneratedBy(type = OverrideBook::class)
public fun ImmutableCreator<OverrideBook>.`by`(resolveImmediately: Boolean = false, block: OverrideBookDraft.() -> Unit): OverrideBook = OverrideBookDraft.`$`.produce(null, resolveImmediately, block)

@GeneratedBy(type = OverrideBook::class)
public fun ImmutableCreator<OverrideBook>.`by`(base: OverrideBook?, resolveImmediately: Boolean = false): OverrideBook = OverrideBookDraft.`$`.produce(base, resolveImmediately)

@GeneratedBy(type = OverrideBook::class)
public fun ImmutableCreator<OverrideBook>.`by`(
    base: OverrideBook?,
    resolveImmediately: Boolean = false,
    block: OverrideBookDraft.() -> Unit,
): OverrideBook = OverrideBookDraft.`$`.produce(base, resolveImmediately, block)

@GeneratedBy(type = OverrideBook::class)
public fun OverrideBook(resolveImmediately: Boolean = false, block: OverrideBookDraft.() -> Unit): OverrideBook = OverrideBookDraft.`$`.produce(null, resolveImmediately, block)

@GeneratedBy(type = OverrideBook::class)
public fun OverrideBook(
    base: OverrideBook?,
    resolveImmediately: Boolean = false,
    block: OverrideBookDraft.() -> Unit,
): OverrideBook = OverrideBookDraft.`$`.produce(base, resolveImmediately, block)

@GeneratedBy(type = OverrideBook::class)
public fun MutableList<OverrideBookDraft>.addBy(resolveImmediately: Boolean = false, block: OverrideBookDraft.() -> Unit): MutableList<OverrideBookDraft> {
    add(OverrideBookDraft.`$`.produce(null, resolveImmediately, block) as OverrideBookDraft)
    return this
}

@GeneratedBy(type = OverrideBook::class)
public fun MutableList<OverrideBookDraft>.addBy(base: OverrideBook?, resolveImmediately: Boolean = false): MutableList<OverrideBookDraft> {
    add(OverrideBookDraft.`$`.produce(base, resolveImmediately) as OverrideBookDraft)
    return this
}

@GeneratedBy(type = OverrideBook::class)
public fun MutableList<OverrideBookDraft>.addBy(
    base: OverrideBook?,
    resolveImmediately: Boolean = false,
    block: OverrideBookDraft.() -> Unit,
): MutableList<OverrideBookDraft> {
    add(OverrideBookDraft.`$`.produce(base, resolveImmediately, block) as OverrideBookDraft)
    return this
}

@GeneratedBy(type = OverrideBook::class)
public fun OverrideBook.copy(resolveImmediately: Boolean = false, block: OverrideBookDraft.() -> Unit): OverrideBook = OverrideBookDraft.`$`.produce(this, resolveImmediately, block)
