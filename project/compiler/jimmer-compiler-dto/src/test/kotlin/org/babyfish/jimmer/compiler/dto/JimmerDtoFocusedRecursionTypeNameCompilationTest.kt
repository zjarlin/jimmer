...
class JimmerDtoFocusedRecursionTypeNameCompilationTest {

    @Test
    fun `apt compiles owner target in every focused recursion scope`() {
        compileApt().assertOwnerTargets(java = true)
    }

    @Test
    fun `ksp compiles owner target in every focused recursion scope`() {
        compileKsp().assertOwnerTargets(java = false)
    }

    private fun compileApt(): String {
        val projectDir = fixtureProject("jimmer-dto-focused-recursion-apt")
        ...
        return generatedDir.resolve("demo/dto/NodeView.java").readText()
    }

    private fun compileKsp(): String {
        val projectDir = fixtureProject("jimmer-dto-focused-recursion-ksp")
        ...
        val configuration = KSPJvmConfig.Builder().apply {
            moduleName = "jimmer-dto-focused-recursion"
        ...
        return kotlinOutputDir.resolve("demo/dto/NodeView.kt").readText()
    }
    ...
    private fun writeDtoSource(projectDir: File) {
        projectDir.resolve("src/main/dto/demo/Node.dto").also { file ->
            file.parentFile.mkdirs()
    ...
    private fun String.assertOwnerTargets(java: Boolean) {
        val declaration = if (java) "class TargetOf_owner" else "class TargetOf_owner("
        assertEquals(3, countOccurrences(declaration), this)
        assertContains(this, "\"NodeView.TargetOf_owner\"")
        assertContains(this, "\"NodeView.TargetOf_parent.TargetOf_owner\"")
        assertContains(this, "\"NodeView.TargetOf_children.TargetOf_owner\"")
    }
