...
public class ImmutableProp implements BaseProp {
    ...
    private final ImmutableProp overriddenProp;
    ...
    public ImmutableProp(
        ...
        this(context, declaringType, executableElement, id, null);
    }
    ...
    public ImmutableProp(
            ...
            ExecutableElement executableElement,
            int id,
            ImmutableProp overriddenProp
    ) {
        this.context = context;
        this.id = id;
        this.declaringType = declaringType;
        this.executableElement = executableElement;
        this.overriddenProp = overriddenProp;
        ...
        if (getAnnotation(ManyToManyView.class) != null) {
            deeperPropIdName = "DEEPER_PROP_ID_" + Strings.upper(name);
        } else {
            deeperPropIdName = null;
        }

        Default defaultAnnotation = getAnnotation(Default.class);
        ...
        if (defaultAnnotation != null && getAnnotation(LogicalDeleted.class) != null) {
        ...
        isDiscriminator = getAnnotation(Discriminator.class) != null;
        ...
        if (isTransient) {
            for (AnnotationMirror mirror : getAnnotations()) {
        ...
        for (AnnotationMirror annotationMirror : getMetadataAnnotations()) {
        ...
        primaryAnnotationType = descriptor.getType().getAnnotationType();
        if (descriptor.getType().isAssociation()) {
            associationAnnotation = getAnnotation(descriptor.getType().getAnnotationType());
        }
        ...
        this.validationMessageMap = ValidationMessages.parseMessageMap(executableElement, getAnnotations());
    }
    ...
    public <A extends Annotation> A getAnnotation(Class<A> annotationType) {
        A annotation = executableElement.getAnnotation(annotationType);
        if (annotation != null || overriddenProp == null || annotationType == Override.class) {
            return annotation;
        }
        return overriddenProp.getAnnotation(annotationType);
    }

    public <A extends Annotation> A[] getAnnotations(Class<A> annotationType) {
        A[] annotations = executableElement.getAnnotationsByType(annotationType);
        if (annotations.length != 0 || overriddenProp == null || annotationType == Override.class) {
            return annotations;
        }
        return overriddenProp.getAnnotations(annotationType);
    }
    ...
    public List<? extends AnnotationMirror> getAnnotations() {
        ...
        return mergeAnnotations(annotations, overriddenProp.getAnnotations());
    }

    private List<? extends AnnotationMirror> getMetadataAnnotations() {
        List<AnnotationMirror> annotations = new ArrayList<>();
        annotations.addAll(executableElement.getAnnotationMirrors());
        annotations.addAll(executableElement.getReturnType().getAnnotationMirrors());
        if (overriddenProp == null) {
            return annotations;
        }
        return mergeAnnotations(annotations, overriddenProp.getMetadataAnnotations());
    }

    private AnnotationMirror recursiveAnnotation(String annotationTypeName) {
        AnnotationMirror annotation = RecursiveAnnotations.of(executableElement, annotationTypeName);
        if (annotation != null || overriddenProp == null) {
            return annotation;
        }
        return overriddenProp.recursiveAnnotation(annotationTypeName);
    }

    private static List<? extends AnnotationMirror> mergeAnnotations(
            List<? extends AnnotationMirror> annotations,
            List<? extends AnnotationMirror> inheritedAnnotations
    ) {
        Set<String> annotationTypeNames = new HashSet<>();
        for (AnnotationMirror annotation : annotations) {
            annotationTypeNames.add(annotationTypeName(annotation));
        }
        List<AnnotationMirror> mergedAnnotations = new ArrayList<>(
                annotations.size() + inheritedAnnotations.size()
        );
        mergedAnnotations.addAll(annotations);
        for (AnnotationMirror inheritedAnnotation : inheritedAnnotations) {
            String inheritedAnnotationTypeName = annotationTypeName(inheritedAnnotation);
            if (!inheritedAnnotationTypeName.equals(Override.class.getName()) &&
                    !annotationTypeNames.contains(inheritedAnnotationTypeName)) {
                mergedAnnotations.add(inheritedAnnotation);
            }
        }
        return Collections.unmodifiableList(mergedAnnotations);
    }

    private static String annotationTypeName(AnnotationMirror annotation) {
