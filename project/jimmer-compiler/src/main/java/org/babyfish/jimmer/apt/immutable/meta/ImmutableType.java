...
public class ImmutableType implements BaseType {
    ...
    public ImmutableType(
        ...
        List<ImmutableProp> propsToValidate = new ArrayList<>();
        ...
        Map<String, ImmutableProp> superPropByGetterName = new HashMap<>();
        for (ImmutableProp prop : superPropMap.values()) {
            superPropByGetterName.put(prop.getGetterName(), prop);
        }
        Map<String, ImmutableProp> redefinedPropByGetterName = new HashMap<>();
        for (ImmutableProp prop : redefinedPropMap.values()) {
            redefinedPropByGetterName.put(prop.getGetterName(), prop);
        }
        Set<String> directlyOverridableGetterNames = new HashSet<>();
        if (isEntity) {
            for (ImmutableType superType : superTypes) {
                if (superType.isMappedSuperClass) {
                    for (ImmutableProp prop : superType.getDeclaredProps().values()) {
                        directlyOverridableGetterNames.add(prop.getGetterName());
                    }
                }
            }
        }
        for (int i = 0; i < 2; i++) {
            for (ExecutableElement executableElement : ElementFilter.methodsIn(typeElement.getEnclosedElements())) {
                if (((executableElement.getAnnotation(Id.class)) != null) == (i == 0)) {
                    ...
                    ImmutableProp redefinedProp = isEntity ? redefinedPropByGetterName.get(getterName) : null;
                    if (redefinedProp != null && directlyOverridableGetterNames.contains(getterName)) {
                        ImmutableProp prop = new ImmutableProp(
                                context,
                                this,
                                executableElement,
                        ...
                        if (!context.getTypes().isSameType(prop.getReturnType(), redefinedProp.getReturnType())) {
                            throw new MetaException(
                                    executableElement,
                                    "its return type is different from inherited property \"" +
                                            redefinedProp +
                                            "\""
                            );
                        }
                        if (!Objects.equals(prop.getPrimaryAnnotationType(), redefinedProp.getPrimaryAnnotationType()) ||
                                prop.isNullable() != redefinedProp.isNullable() ||
                                prop.isList() != redefinedProp.isList() ||
                                prop.isAssociation(false) != redefinedProp.isAssociation(false) ||
                                prop.isJavaFormula() != redefinedProp.isJavaFormula()) {
                            throw new MetaException(
                                    executableElement,
                                    "it can override annotations of inherited property \"" +
                                            redefinedProp +
                                            "\", but cannot change its category, nullability or primary mapping annotation"
                            );
                        }
                        propsToValidate.remove(redefinedProp);
                        propsToValidate.add(prop);
                        redefinedPropMap.put(prop.getName(), prop);
                        continue;
                    }
                    if (superPropByGetterName.containsKey(getterName)) {
                        throw new MetaException(
                                executableElement,
        ...
        for (ImmutableProp prop : propsToValidate) {
            ExecutableElement executableElement = prop.toElement();
            if (executableElement.isDefault()) {
                for (AnnotationMirror am : prop.getAnnotations()) {
        ...
        for (ImmutableProp prop : propsToValidate) {
            ExecutableElement executableElement = prop.toElement();
            Formula formula = prop.getAnnotation(Formula.class);
