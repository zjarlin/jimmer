...
class ImmutableType(
    ...
    init {
        ...
        val declaredPropDeclarations = classDeclaration.getDeclaredProperties().toList()
        ...
        val overridingPropDeclarationMap = mutableMapOf<String, KSPropertyDeclaration>()
        val directlyOverridablePropNames = superTypes
            .asSequence()
            .filter(ImmutableType::isMappedSuperclass)
            .flatMap { superType -> superType.declaredProperties.keys.asSequence() }
            .toSet()
        for (propDeclaration in declaredPropDeclarations) {
            val superProp = superPropMap[propDeclaration.name]
            if (superProp != null) {
                if (!isEntity ||
                    !superProp.declaringType.isMappedSuperclass ||
                    propDeclaration.name !in directlyOverridablePropNames
                ) {
                    throw MetaException(
                        propDeclaration,
                        "it overrides '$superProp', only properties inherited directly from mapped superclasses " +
                            "can be overridden"
                    )
                }
                overridingPropDeclarationMap[propDeclaration.name] = propDeclaration
            }
        }
        ...
        }.mapValues { (name, superProp) ->
            val overridingPropDeclaration = overridingPropDeclarationMap[name]
            if (overridingPropDeclaration != null) {
                val inheritedProp = ImmutableProp(
                    ctx = ctx,
                    declaringType = this,
                    id = propIdSequence,
                    propDeclaration = superProp.propDeclaration,
                    inherited = true,
                    overriddenProp = superProp,
                )
                ImmutableProp(
                    ctx = ctx,
                    declaringType = this,
                    id = propIdSequence++,
                    propDeclaration = overridingPropDeclaration,
                    overriddenProp = superProp,
                ).also { prop ->
                    if (prop.resolvedType.makeNotNullable() != inheritedProp.resolvedType.makeNotNullable()) {
                        throw MetaException(
                            overridingPropDeclaration,
                            "its return type '${prop.resolvedType}' does not match the inherited property " +
                                "type '${inheritedProp.resolvedType}'"
                        )
                    }
                    if (prop.primaryAnnotationType != inheritedProp.primaryAnnotationType ||
                        prop.isNullable != inheritedProp.isNullable ||
                        prop.isList != inheritedProp.isList ||
                        prop.isAssociation(false) != inheritedProp.isAssociation(false) ||
                        prop.isKotlinFormula != inheritedProp.isKotlinFormula
                    ) {
                        throw MetaException(
                            overridingPropDeclaration,
                            "it can override annotations of inherited property '$superProp', " +
                                "but cannot change its category, nullability, formula kind or primary mapping annotation"
                        )
                    }
                }
            } else if (isMappedSuperclass && superProp.declaringType.isMappedSuperclass) {
                superProp
            } else {
                ImmutableProp(
                    ctx = ctx,
        ...
        declaredProperties =
            declaredPropDeclarations
                .asSequence()
                .filterNot { propDeclaration ->
                    propDeclaration.name in overridingPropDeclarationMap
                }
        ...
        for (prop in declaredProperties.values) {
            validateDeclaredProperty(prop)
        }
        for (name in overridingPropDeclarationMap.keys) {
            redefinedProps[name]?.let(::validateDeclaredProperty)
        }
    }

    private fun validateDeclaredProperty(prop: ImmutableProp) {
        val propDeclaration = prop.propDeclaration
        val formula = prop.annotation(Formula::class)
        ...
        for (annotation in prop.annotations { true }) {
            if (annotation.fullName.startsWith("org.babyfish.jimmer.") &&
                annotation.fullName != FORMULA_CLASS_NAME
            ) {
                throw MetaException(
                    propDeclaration,
        ...
        if (formula !== null) {
            formula[Formula::sql]?.takeIf(String::isNotEmpty)?.let {
                throw MetaException(
                    propDeclaration,
