...
class ImmutableProp(
    ...
    private val ownDeclarationAnnotations: List<KSAnnotation> by lazy {
        propDeclaration.annotations.toList()
    }

    private val ownGetterAnnotations: List<KSAnnotation> by lazy {
        propDeclaration.getter?.annotations?.toList().orEmpty()
    }

    private val ownTypeAnnotations: List<KSAnnotation> by lazy {
        propDeclaration.type.annotations.toList()
    }

    private val ownAnnotations: List<KSAnnotation> by lazy {
        ownDeclarationAnnotations + ownGetterAnnotations + ownTypeAnnotations
    }

    private val ownAnnotationTypeNames: Set<String> by lazy {
        ownAnnotations.mapTo(mutableSetOf()) { annotation -> annotation.fullName }
    }

    val isGenericSourceTarget: Boolean =
        inherited && sourceTargetType(sourceResolvedType).declaration is KSTypeParameter ||
            overriddenProp?.let { prop ->
                prop.isGenericSourceTarget ||
                    sourceTargetType(prop.sourceResolvedType).declaration is KSTypeParameter
            } == true
    ...
    init {
        ...
        val defaultAnnotation = annotation(Default::class)
        val databaseDefault = annotation(DatabaseDefault::class)
        ...
        if (annotation(LogicalDeleted::class) !== null) {
    ...
    init {
        val descriptor = PropDescriptor
            .newBuilder(
                ...
                for (annotation in annotations { true }) {
    ...
    fun annotations(annotationType: KClass<out Annotation>): List<KSAnnotation> {
        val annotationTypeName = annotationType.qualifiedName
        val annotations = ownAnnotations.filter { annotation ->
            annotation.fullName == annotationTypeName
        }
        return annotations.takeIf(List<KSAnnotation>::isNotEmpty)
            ?: overriddenProp?.annotations(annotationType)
            ?: emptyList()
    }

    fun annotations(predicate: (KSAnnotation) -> Boolean): List<KSAnnotation> {
        return mergedAnnotations(
            ownAnnotations.filter(predicate),
            overriddenProp?.annotations(predicate).orEmpty(),
        )
    }

    private fun recursiveAnnotationOf(annotationTypeName: String): KSAnnotation? =
        propDeclaration.recursiveAnnotationOf(annotationTypeName)
            ?: overriddenProp?.recursiveAnnotationOf(annotationTypeName)
    ...
    val getterAnnotations: List<KSAnnotation>
        get() = mergedAnnotations(
            ownGetterAnnotations,
            overriddenProp?.getterAnnotations.orEmpty(),
        )

    val typeAnnotations: List<KSAnnotation>
        get() = mergedAnnotations(
            ownTypeAnnotations,
            overriddenProp?.typeAnnotations.orEmpty(),
        )

    val validationMessages: Map<ClassName, String> =
        overriddenProp?.validationMessages.orEmpty() + parseValidationMessages(propDeclaration)

    private fun mergedAnnotations(
        annotations: List<KSAnnotation>,
        inheritedAnnotations: List<KSAnnotation>,
    ): List<KSAnnotation> {
        if (inheritedAnnotations.isEmpty()) {
            return annotations
        }
        return annotations + inheritedAnnotations.filterNot { annotation ->
            annotation.fullName in ownAnnotationTypeNames
        }
    }
