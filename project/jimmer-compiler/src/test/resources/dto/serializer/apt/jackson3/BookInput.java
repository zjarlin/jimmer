...
public class BookInput implements Input<Book> {
    ...
    public static class TargetOf_location implements EmbeddableDto<Point> {
        ...
        public static class Serializer extends ValueSerializer<TargetOf_location> {
            @Override
            public void serialize(TargetOf_location input, JsonGenerator gen,
                    SerializationContext provider) {
                gen.writeStartObject();
                if (input.isXLoaded()) {
                    provider.defaultSerializeProperty("x", input.getX(), gen);
                }
                if (input.isYLoaded()) {
                    provider.defaultSerializeProperty("y", input.getY(), gen);
                }
                gen.writeEndObject();
            }
        }
    ...
    public static class Serializer extends ValueSerializer<BookInput> {
        @Override
        public void serialize(BookInput input, JsonGenerator gen, SerializationContext provider) {
            gen.writeStartObject();
            provider.defaultSerializeProperty("id", input.getId(), gen);
            provider.defaultSerializeProperty("isEnabled", input.isEnabled(), gen);
            provider.defaultSerializeProperty("name", input.getName(), gen);
            if (input.isEditionLoaded()) {
                provider.defaultSerializeProperty("edition", input.getEdition(), gen);
            }
            provider.defaultSerializeProperty("price", input.getPrice(), gen);
            if (input.isLocationLoaded()) {
                provider.defaultSerializeProperty("location", input.getLocation(), gen);
            }
            gen.writeEndObject();
        }
    }
