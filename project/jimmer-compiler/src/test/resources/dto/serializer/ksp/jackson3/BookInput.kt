...
public open class BookInput(
    ...
    public open class TargetOf_location(
        ...
        public class Serializer : ValueSerializer<TargetOf_location>() {
            override fun serialize(
                ...
                if (input.isXLoaded) {
                    provider.defaultSerializeProperty("x", input.x, gen)
                }
                if (input.isYLoaded) {
                    provider.defaultSerializeProperty("y", input.y, gen)
                }
                gen.writeEndObject()
            }
        }
    ...
    public class Serializer : ValueSerializer<BookInput>() {
        override fun serialize(
            input: BookInput,
            gen: JsonGenerator,
            provider: SerializationContext,
            ...
            gen.writeStartObject()
            provider.defaultSerializeProperty("id", input.id, gen)
            provider.defaultSerializeProperty("isEnabled", input.isEnabled, gen)
            provider.defaultSerializeProperty("name", input.name, gen)
            if (input.isEditionLoaded) {
                provider.defaultSerializeProperty("edition", input.edition, gen)
            }
            provider.defaultSerializeProperty("price", input.price, gen)
            if (input.isLocationLoaded) {
                provider.defaultSerializeProperty("location", input.location, gen)
            }
            gen.writeEndObject()
        }
    }
    ...
    @GeneratedBy
    @JsonPOJOBuilder(withPrefix = "")
    @ToolsJacksonDatabindAnnotationJsonNaming(value = ToolsJacksonDatabindPropertyNamingStrategies.SnakeCaseStrategy::class)
    public class Builder {
