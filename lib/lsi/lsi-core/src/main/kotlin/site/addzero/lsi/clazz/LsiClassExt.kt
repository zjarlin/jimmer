...
fun LsiClass.annotations(predicate: (LsiAnnotation) -> Boolean): List<LsiAnnotation> =
    annotations.filter(predicate)

fun LsiClass.annotations(annotationType: KClass<out Annotation>): List<LsiAnnotation> =
    annotations { it.qualifiedName == annotationType.qualifiedName }

fun LsiClass.annotation(annotationType: KClass<out Annotation>): LsiAnnotation? =
    annotation(annotationType.qualifiedName!!)


fun LsiClass.annotation(qualifiedName: String): LsiAnnotation? =
    annotations.firstOrNull { it.qualifiedName == qualifiedName }

fun LsiClass.packageAnnotations(predicate: (LsiAnnotation) -> Boolean): List<LsiAnnotation> =
    packageAnnotations.filter(predicate)

fun LsiClass.packageAnnotation(annotationType: KClass<out Annotation>): LsiAnnotation? =
    packageAnnotation(annotationType.qualifiedName!!)

fun LsiClass.packageAnnotation(qualifiedName: String): LsiAnnotation? =
    packageAnnotations.firstOrNull { it.qualifiedName == qualifiedName }
